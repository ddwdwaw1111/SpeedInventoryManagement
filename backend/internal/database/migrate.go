...
func applyBaselineSchemaMigration(db *sql.DB) error {
	statements := []string{
		...
		`ALTER TABLE outbound_document_lines ADD COLUMN IF NOT EXISTS planned_quantity INT NOT NULL DEFAULT 0 AFTER quantity`,
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_bulk_import_batch_documents_key ON bulk_import_batch_documents (batch_id, document_key)`,
