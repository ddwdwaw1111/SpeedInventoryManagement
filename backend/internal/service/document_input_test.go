...
func TestSanitizeInboundDocumentInput(t *testing.T) {
	input := sanitizeInboundDocumentInput(CreateInboundDocumentInput{
		...
		Lines: []CreateInboundDocumentLineInput{
			{ItemNumber: " item-100 ", SKU: " sku-1 ", Description: "  Pan  ", StorageSection: " b ", LineNote: "  keep cold ", ExpectedQty: 10, ReorderLevel: 9, InboundCtnsPerPallet: 4},
	...
	if input.Lines[0].InboundCtnsPerPallet != 4 {
		t.Fatalf("expected CTN per pallet to be preserved as receipt metadata, got %d", input.Lines[0].InboundCtnsPerPallet)
	}
}
...
func TestConfirmedInboundKeepsQuantityAndPalletCountIndependent(t *testing.T) {
	input := CreateInboundDocumentInput{
		...
		Lines: []CreateInboundDocumentLineInput{
			{
				SKU: "SKU-1", ExpectedQty: 10, ReceivedQty: 10, Pallets: 12, InboundCtnsPerPallet: 4,
	...
	input.Lines[0].InboundCtnsPerPallet = 0
	if err := validateInboundDocumentInput(input); err != nil {
