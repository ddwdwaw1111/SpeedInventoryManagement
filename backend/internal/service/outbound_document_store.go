...
type OutboundPickAllocation struct {
	...
	TargetPallets  int       `json:"targetPallets"`
	CreatedAt      time.Time `json:"createdAt"`

	sourcePalletsSet bool
	targetPalletsSet bool
}

type outboundPickAllocationJSON struct {
	ID             int64     `json:"id"`
	LineID         int64     `json:"lineId"`
	ItemNumber     string    `json:"itemNumber"`
	LocationID     int64     `json:"locationId"`
	...
	AllocatedQty   int       `json:"allocatedQty"`
	Pallets        int       `json:"pallets"`
	SourcePallets  *int      `json:"sourcePallets,omitempty"`
	TargetPallets  *int      `json:"targetPallets,omitempty"`
	CreatedAt      time.Time `json:"createdAt"`
}

func (allocation OutboundPickAllocation) MarshalJSON() ([]byte, error) {
	payload := outboundPickAllocationJSON{
		ID:             allocation.ID,
		LineID:         allocation.LineID,
		ItemNumber:     allocation.ItemNumber,
	...
	if outboundPickAllocationHasSourcePallets(allocation) {
		value := allocation.SourcePallets
		payload.SourcePallets = &value
	}
	if outboundPickAllocationHasTargetPallets(allocation) {
		value := allocation.TargetPallets
		payload.TargetPallets = &value
	}
	return json.Marshal(payload)
}

func (allocation *OutboundPickAllocation) UnmarshalJSON(data []byte) error {
	var payload outboundPickAllocationJSON
	if err := json.Unmarshal(data, &payload); err != nil {
		return err
	}

	allocation.ID = payload.ID
	allocation.LineID = payload.LineID
	allocation.ItemNumber = payload.ItemNumber
	allocation.LocationID = payload.LocationID
	allocation.LocationName = payload.LocationName
	allocation.StorageSection = payload.StorageSection
	allocation.ContainerID = payload.ContainerID
	allocation.ContainerNo = payload.ContainerNo
	allocation.AllocatedQty = payload.AllocatedQty
	allocation.Pallets = payload.Pallets
	allocation.SourcePallets = 0
	allocation.TargetPallets = 0
	allocation.CreatedAt = payload.CreatedAt
	allocation.sourcePalletsSet = payload.SourcePallets != nil
	allocation.targetPalletsSet = payload.TargetPallets != nil
	if payload.SourcePallets != nil {
		allocation.SourcePallets = *payload.SourcePallets
	}
	if payload.TargetPallets != nil {
		allocation.TargetPallets = *payload.TargetPallets
	}
...
func (s *Store) ListOutboundDocumentsFiltered(ctx context.Context, limit int, filters OutboundDocumentFilters) ([]OutboundDocument, error) {
	...
	for _, lineRow := range lineRows {
		...
		document.Lines = append(document.Lines, OutboundDocumentLine{
			...
			PickAllocations:   decodeOutboundPickAllocationsForLine(lineRow.ID, lineRow.PickAllocationsJSON),
...
func (s *Store) UpdateOutboundDocument(ctx context.Context, documentID int64, input CreateOutboundDocumentInput) (OutboundDocument, error) {
	...
	if outboundTrackingRequiresActiveReservation(existingTrackingStatus) {
		if err := s.releaseOutboundDocumentReservationsTx(ctx, tx, documentRow.CustomerID, existingLineRows); err != nil {
			return OutboundDocument{}, err
		}
	}
...
func (s *Store) insertOutboundDocumentLinesTx(ctx context.Context, tx *sql.Tx, documentID int64, input CreateOutboundDocumentInput, lockedSources map[string]lockedOutboundSource) error {
	...
	for index, line := range input.Lines {
		...
		pickAllocations := line.PickAllocations
		if len(pickAllocations) == 0 && len(allocations) > 0 {
			pickAllocations = toOutboundPickAllocationsFromCandidates(&line, allocations)
		}
		...
		`,
			...
			nullableString(mustEncodeOutboundPickAllocations(pickAllocations)),
			index+1,
		)
		if err != nil {
...
func (s *Store) confirmOutboundDocumentTx(ctx context.Context, tx *sql.Tx, documentID int64, finalTrackingStatus ...string) error {
	...
	for _, lineRow := range lineRows {
		selectedAllocations := decodeOutboundPickAllocationsOrEmpty(lineRow.PickAllocationsJSON)
		if len(selectedAllocations) == 0 {
			return fmt.Errorf("%w: shipment must be reserved before it can be shipped", ErrInvalidInput)
		}
		totalSelectedQty := totalOutboundPickAllocationQuantity(selectedAllocations)
		if totalSelectedQty != lineRow.Quantity {
			return fmt.Errorf("%w: selected inventory quantity must equal outbound quantity", ErrInvalidInput)
		}
		effectiveLinePallets := lineRow.Pallets
		if effectiveLinePallets <= 0 {
			effectiveLinePallets = totalOutboundPickAllocationPallets(selectedAllocations)
		}

		allocationCandidates := toOutboundAllocationCandidatesFromDraftPickAllocations(lockedOutboundSource{
			SKUMasterID:  lineRow.SKUMasterID,
			...
			SKU:          lineRow.SKUSnapshot,
			Description:  lineRow.DescriptionSnapshot,
			Unit:         lineRow.UnitLabel,
			Quantity:     lineRow.Quantity,
			AvailableQty: lineRow.Quantity,
		...
		palletSplits := splitOutboundLinePalletsByAllocations(effectiveLinePallets, selectedAllocations)

		for allocationIndex, allocation := range selectedAllocations {
			...
			containerID := allocation.ContainerID
			targetPallets := firstPositiveInt(allocation.TargetPallets, allocation.Pallets)
			if targetPallets <= 0 && allocationIndex < len(palletSplits) {
				targetPallets = palletSplits[allocationIndex]
			}
			...
			currentBalance, err := s.loadLockedInventoryBalanceForBucketTx(ctx, tx, bucket)
			if err != nil {
				return fmt.Errorf("load reserved inventory balance for outbound movement: %w", err)
			}
			containerID = firstNonZeroInt64(containerID, currentBalance.ContainerID)
			containerNo = firstNonEmpty(currentBalance.ContainerNo, containerNo)
			bucket.ContainerID = containerID
			bucket.ContainerNo = containerNo

			sourcePallets := allocation.SourcePallets
			if !outboundPickAllocationHasSourcePallets(allocation) {
				sourcePallets = deriveOutboundSourcePallets(currentBalance.Pallets, currentBalance.Quantity, allocation.AllocatedQty)
			}

			selectedAllocations[allocationIndex].LocationID = locationID
			selectedAllocations[allocationIndex].StorageSection = storageSection
			selectedAllocations[allocationIndex].ContainerID = containerID
			selectedAllocations[allocationIndex].ContainerNo = containerNo
			selectedAllocations[allocationIndex].Pallets = targetPallets
			selectedAllocations[allocationIndex].SourcePallets = sourcePallets
			selectedAllocations[allocationIndex].TargetPallets = targetPallets

			if err := s.consumeReservedInventoryBalanceTx(ctx, tx, bucket, allocation.AllocatedQty, sourcePallets); err != nil {
				return fmt.Errorf("ship reserved inventory balance for outbound movement: %w", err)
			}

			_, err = s.createStockLedgerEntryTx(ctx, tx, createStockLedgerInput{
				EventType:           StockLedgerEventShip,
				OccurredAt:          outboundEventTime,
				SKUMasterID:         lineRow.SKUMasterID,
				CustomerID:          documentRow.CustomerID,
				LocationID:          locationID,
				StorageSection:      storageSection,
				...
				Pallets:             sourcePallets,
				PalletsDetailCtns:   lineRow.PalletsDetailCtns,
				CartonSizeMM:        lineRow.CartonSizeMM,
				CartonCount:         allocation.AllocatedQty,
				UnitLabel:           firstNonEmpty(lineRow.UnitLabel, "PLT"),
		...
		if effectiveLinePallets > 0 {
			if _, err := tx.ExecContext(ctx, `
				...
				SET pallets = ?, pick_allocations_json = ?
				WHERE id = ?
			`, effectiveLinePallets, nullableString(mustEncodeOutboundPickAllocations(selectedAllocations)), lineRow.ID); err != nil {
				return mapDBError(fmt.Errorf("update outbound line allocation pallets: %w", err))
			}
		}
	}
...
func (s *Store) CancelOutboundDocument(ctx context.Context, documentID int64) (OutboundDocument, error) {
	...
	if status == DocumentStatusConfirmed {
		if err := s.restoreInventoryBalancesForOutboundLedgerTx(ctx, tx, documentID); err != nil {
			return OutboundDocument{}, err
		}
	...
	// Delete outbound document and its lines. Pallet-level pick rows are legacy-only.
...
func (s *Store) reserveOutboundLineTx(
	...
	if len(line.PickAllocations) > 0 {
		...
		allocations, err := s.resolveOutboundLineAllocationsTx(ctx, tx, source, line.Quantity, reservationState)
		if err != nil {
			return err
		}
		plannedAllocations = toOutboundPickAllocationsFromCandidates(line, allocations)
	...
	for _, allocation := range plannedAllocations {
		...
		if err := s.reserveInventoryBalanceTx(ctx, tx, bucket, allocation.AllocatedQty); err != nil {
			return err
		}
	}

	line.PickAllocations = normalizeOutboundPickAllocations(plannedAllocations)
	return nil
}
...
func (s *Store) reserveOutboundDocumentLinesTx(
	...
	lockedSources := make(map[string]lockedOutboundSource)
	palletReservationState := newOutboundAllocationReservationState()
	for index := range lineRows {
		...
		lineReservationState := newOutboundAllocationReservationState()
		lineReservationState.PalletsByBucketKey = palletReservationState.PalletsByBucketKey
		if err := s.reserveOutboundLineTx(ctx, tx, lockedSource, &lineInput, lineReservationState); err != nil {
			return nil, err
		}
	...
	for _, lineRow := range lineRows {
		for _, allocation := range decodeOutboundPickAllocationsOrEmpty(lineRow.PickAllocationsJSON) {
			...
			if err := s.releaseInventoryReservationTx(ctx, tx, inventorySourceBucket{
				...
				ContainerID:    allocation.ContainerID,
				ContainerNo:    allocation.ContainerNo,
			}, allocation.AllocatedQty); err != nil {
				return err
			}
		}
	}
...
func (s *Store) outboundDocumentReservationsSatisfiedTx(ctx context.Context, tx *sql.Tx, customerID int64, lineRows []outboundDocumentLineRow) (bool, error) {
	for _, lineRow := range lineRows {
		...
		for _, allocation := range allocations {
			...
			if state.AllocatedQty < allocation.AllocatedQty {
				return false, nil
			}
		}
	}
...
func (s *Store) repairOutboundDraftReservations(ctx context.Context) error {
	...
	rows, err := tx.QueryContext(ctx, `
		...
		ORDER BY id ASC
	`, DocumentStatusDraft, OutboundTrackingPicking, OutboundTrackingPacked)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&documentID); err != nil {
			return fmt.Errorf("scan outbound draft for reservation repair: %w", err)
		}
		documentIDs = append(documentIDs, documentID)
	}
	if err := rows.Err(); err != nil {
		return fmt.Errorf("iterate outbound drafts for reservation repair: %w", err)
	}
	if err := rows.Close(); err != nil {
		return fmt.Errorf("close outbound drafts for reservation repair: %w", err)
	}

	if err := s.resetOutboundDraftReservationStateTx(ctx, tx, documentIDs); err != nil {
		return err
	}

	for _, documentID := range documentIDs {
		if err := s.repairSingleOutboundDraftReservationTx(ctx, tx, documentID); err != nil {
			return err
		}
	}
...
func (s *Store) resetOutboundDraftReservationStateTx(ctx context.Context, tx *sql.Tx, documentIDs []int64) error {
	if len(documentIDs) == 0 {
		return nil
	}

	bucketsByKey := make(map[string]outboundRepairBucketReservation)
	...
	for _, documentID := range documentIDs {
		...
		for _, lineRow := range lineRows {
			for _, allocation := range decodeOutboundPickAllocationsOrEmpty(lineRow.PickAllocationsJSON) {
				...
				addOutboundRepairBucketReservation(bucketsByKey, bucket, allocation.AllocatedQty)
			}

		}
	}

	for _, reservation := range bucketsByKey {
		...
		if _, err := tx.ExecContext(ctx, `
			...
			SET allocated_qty = GREATEST(allocated_qty - ?, 0), updated_at = CURRENT_TIMESTAMP
			WHERE sku_master_id = ?
				AND customer_id = ?
				AND location_id = ?
		...
		`,
			...
			fallbackSection(bucket.StorageSection),
			bucket.ContainerID,
			bucket.ContainerID,
			bucket.ContainerID,
			strings.TrimSpace(bucket.ContainerNo),
		); err != nil {
			return mapDBError(fmt.Errorf("reset inventory reservations: %w", err))
		}
	}
...
func addOutboundRepairBucketReservation(reservations map[string]outboundRepairBucketReservation, bucket inventorySourceBucket, quantity int) {
	...
	key := outboundAllocationBucketKey(bucket.CustomerID, bucket.LocationID, bucket.SKUMasterID, bucket.StorageSection, bucket.ContainerID, bucket.ContainerNo)
	existing := reservations[key]
	if existing.Bucket.SKUMasterID == 0 {
...
func (s *Store) repairSingleOutboundDraftReservationTx(ctx context.Context, tx *sql.Tx, documentID int64) error {
	documentRow, err := s.loadOutboundDocumentForUpdateTx(ctx, tx, documentID)
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			return nil
	...
	reservationsSatisfied, err := s.outboundDocumentReservationsSatisfiedTx(ctx, tx, documentRow.CustomerID, lineRows)
	if err != nil {
		return err
	}
	...
	if _, err := s.reserveOutboundDocumentLinesTx(ctx, tx, documentRow.CustomerID, lineRows); err != nil {
		if _, recoveryErr := tx.ExecContext(ctx, `
			UPDATE outbound_documents
			SET tracking_status = ?, updated_at = CURRENT_TIMESTAMP
			WHERE id = ?
...
func outboundAllocationBucketKey(customerID int64, locationID int64, skuMasterID int64, storageSection string, containerID int64, containerNo string) string {
	containerKey := fmt.Sprintf("id:%d", containerID)
	if containerID <= 0 {
		containerKey = "no:" + strings.TrimSpace(containerNo)
	}
	return fmt.Sprintf(
		"%d|%d|%d|%s|%s",
		customerID,
...
func (s *Store) loadLockedOutboundAllocationCandidatesTx(ctx context.Context, tx *sql.Tx, source lockedOutboundSource) ([]outboundAllocationCandidate, error) {
	rows, err := tx.QueryContext(ctx, `
		SELECT
			...
			COALESCE(i.pallets, 0) AS pallets,
...
func (s *Store) resolveOutboundDraftBucketAllocationsTx(
	...
	for _, candidate := range candidates {
		...
		legacyKey := outboundAllocationBucketKey(candidate.CustomerID, candidate.LocationID, candidate.SKUMasterID, candidate.StorageSection, 0, candidate.ContainerNo)
		if _, exists := candidateByBucketKey[legacyKey]; !exists {
			candidateByBucketKey[legacyKey] = candidate
		}
	}
	...
	for _, draftAllocation := range normalizedDraftAllocations {
		...
		containerID := draftAllocation.ContainerID
		containerNo := strings.TrimSpace(draftAllocation.ContainerNo)
		bucketKey := outboundAllocationBucketKey(source.CustomerID, locationID, source.SKUMasterID, storageSection, containerID, containerNo)
		...
		if !exists {
			for _, applied := range appliedReservations {
				...
				reservationState.PalletsByBucketKey[applied.BucketKey] -= applied.Pallets
			}
			return nil, ErrInsufficientStock
		}
		resolvedBucketKey := candidate.BucketKey

		effectiveAvailable := candidate.AvailableQty - reservationState.ByBucketKey[resolvedBucketKey]
		...
		remainingSourcePallets := maxInt(candidate.SourcePallets-reservationState.PalletsByBucketKey[resolvedBucketKey], 0)
		candidate.SourcePalletsSet = outboundPickAllocationHasSourcePallets(draftAllocation)
		if candidate.SourcePalletsSet {
			candidate.SourcePallets = minInt(draftAllocation.SourcePallets, remainingSourcePallets)
		} else {
			candidate.SourcePallets = deriveOutboundSourcePallets(remainingSourcePallets, effectiveAvailable, draftAllocation.AllocatedQty)
		}
		candidate.Pallets = maxInt(firstPositiveInt(draftAllocation.TargetPallets, draftAllocation.Pallets), 0)

		allocations = append(allocations, candidate)
		reservationState.ByBucketKey[resolvedBucketKey] += draftAllocation.AllocatedQty
		reservationState.PalletsByBucketKey[resolvedBucketKey] += candidate.SourcePallets
		appliedReservations = append(appliedReservations, struct {
			...
			Pallets:   candidate.SourcePallets,
		})
	}
...
func (s *Store) allocateOutboundLineTx(ctx context.Context, tx *sql.Tx, source lockedOutboundSource, requestedQty int, reservationState *outboundAllocationReservationState) ([]outboundAllocationCandidate, error) {
	...
	for _, candidate := range candidates {
		...
		remainingSourcePallets := maxInt(candidate.SourcePallets-reservationState.PalletsByBucketKey[candidate.BucketKey], 0)
		candidate.SourcePallets = deriveOutboundSourcePallets(remainingSourcePallets, effectiveAvailable, allocatedQty)
		allocations = append(allocations, candidate)
		reservationState.ByBucketKey[candidate.BucketKey] += allocatedQty
		reservationState.PalletsByBucketKey[candidate.BucketKey] += candidate.SourcePallets
		appliedReservations = append(appliedReservations, struct {
			BucketKey string
			Allocated int
			Pallets   int
...
func (s *Store) attachOutboundPickAllocations(ctx context.Context, linesByID map[int64]*OutboundDocumentLine) error {
	...
	for _, allocationRow := range ledgerAllocationRows {
		...
		line.PickAllocations = append(line.PickAllocations, OutboundPickAllocation{
			...
			ContainerID:    allocationRow.ContainerID,
			ContainerNo:    allocationRow.ContainerNo,
			AllocatedQty:   allocationRow.AllocatedQty,
			Pallets:        allocationRow.Pallets,
			SourcePallets:  allocationRow.Pallets,
			TargetPallets:  allocationRow.Pallets,
...
func (s *Store) listOutboundLedgerAllocationRowsByLineIDs(ctx context.Context, lineIDs []int64) ([]outboundPickAllocationRow, error) {
	...
	query, args, err := sqlx.In(`
		SELECT
			...
			COALESCE(SUM(sl.pallets), 0) AS pallets,
...
func validateOutboundDocumentInput(input CreateOutboundDocumentInput) error {
	...
	for _, line := range input.Lines {
		...
		case line.Pallets <= 0:
			return fmt.Errorf("%w: outbound pallet count must be greater than zero", ErrInvalidInput)
...
func normalizeOutboundPickAllocations(entries []OutboundPickAllocation) []OutboundPickAllocation {
	...
	for _, entry := range entries {
		...
		key := outboundAllocationBucketKey(0, locationID, 0, storageSection, containerID, containerNo) + "|" + itemNumber

		targetPalletsSet := outboundPickAllocationHasTargetPallets(entry)
		targetPallets := firstPositiveInt(entry.TargetPallets, entry.Pallets)
		sourcePalletsSet := outboundPickAllocationHasSourcePallets(entry)
		sourcePallets := entry.SourcePallets
		if !sourcePalletsSet && !targetPalletsSet {
			sourcePallets = entry.Pallets
		}
		...
		if !exists {
			...
			grouped[key] = &groupedAllocation{
				...
				Pallets:        maxInt(targetPallets, 0),
				SourcePallets:  maxInt(sourcePallets, 0),
				SourcePalletsSet: sourcePalletsSet,
				TargetPallets:  maxInt(targetPallets, 0),
				TargetPalletsSet: targetPalletsSet,
		...
		existing.Pallets += maxInt(targetPallets, 0)
		existing.SourcePallets += maxInt(sourcePallets, 0)
		existing.SourcePalletsSet = existing.SourcePalletsSet || sourcePalletsSet
		existing.TargetPallets += maxInt(targetPallets, 0)
		existing.TargetPalletsSet = existing.TargetPalletsSet || targetPalletsSet
	...
	for _, key := range order {
		...
		normalized = append(normalized, OutboundPickAllocation{
			...
			SourcePallets:  maxInt(entry.SourcePallets, 0),
			TargetPallets:  maxInt(firstPositiveInt(entry.TargetPallets, entry.Pallets), 0),
			CreatedAt:      entry.CreatedAt,
			sourcePalletsSet: entry.SourcePalletsSet,
			targetPalletsSet: entry.TargetPalletsSet,
		})
	}
	return normalized
}
...
func decodeOutboundPickAllocationsForLine(lineID int64, raw string) []OutboundPickAllocation {
...
func totalOutboundPickAllocationPallets(entries []OutboundPickAllocation) int {
	total := 0
	for _, entry := range entries {
		total += maxInt(entry.Pallets, 0)
	}
	return total
}

func splitOutboundLinePalletsByAllocations(totalPallets int, allocations []OutboundPickAllocation) []int {
	result := make([]int, len(allocations))
	if totalPallets <= 0 || len(allocations) == 0 || totalOutboundPickAllocationPallets(allocations) > 0 {
		return result
	}
	...
	for index, allocation := range allocations {
		if allocation.AllocatedQty > 0 {
			totalQty += allocation.AllocatedQty
	...
	remainingPallets := totalPallets
	remainingQty := totalQty
	for index, allocation := range allocations {
		if allocation.AllocatedQty <= 0 {
			continue
		}
		if index == lastPositiveIndex || remainingQty <= 0 {
			result[index] = remainingPallets
			break
		}

		share := remainingPallets * allocation.AllocatedQty / remainingQty
...
func toOutboundAllocationCandidatesFromDraftPickAllocations(source lockedOutboundSource, entries []OutboundPickAllocation) []outboundAllocationCandidate {
	...
	for _, entry := range normalizeOutboundPickAllocations(entries) {
		allocations = append(allocations, outboundAllocationCandidate{
			BucketKey:      outboundAllocationBucketKey(source.CustomerID, firstNonZeroInt64(entry.LocationID, source.LocationID), source.SKUMasterID, entry.StorageSection, entry.ContainerID, entry.ContainerNo),
			SKUMasterID:    source.SKUMasterID,
			CustomerID:     source.CustomerID,
			...
			SourcePallets:  maxInt(entry.SourcePallets, 0),
			SourcePalletsSet: outboundPickAllocationHasSourcePallets(entry),
			Pallets:        maxInt(firstPositiveInt(entry.TargetPallets, entry.Pallets), 0),
		})
	}
	return allocations
}
...
func toOutboundPickAllocationsFromCandidates(line *CreateOutboundDocumentLineInput, allocations []outboundAllocationCandidate) []OutboundPickAllocation {
	...
	for _, allocation := range allocations {
		...
		pickAllocations = append(pickAllocations, OutboundPickAllocation{
			...
			SourcePallets:  maxInt(allocation.SourcePallets, 0),
			TargetPallets:  maxInt(allocation.Pallets, 0),
			CreatedAt:      createdAt,
			sourcePalletsSet: allocation.SourcePalletsSet || allocation.SourcePallets > 0,
			targetPalletsSet: allocation.Pallets > 0,
		})
	}
	if line != nil && line.Pallets > 0 && totalOutboundPickAllocationPallets(pickAllocations) == 0 {
		palletSplits := splitOutboundLinePalletsByAllocations(line.Pallets, pickAllocations)
		for index := range pickAllocations {
			if index < len(palletSplits) {
				pickAllocations[index].Pallets = palletSplits[index]
				pickAllocations[index].TargetPallets = palletSplits[index]
			}
		}
	}
...
func outboundPickAllocationHasSourcePallets(allocation OutboundPickAllocation) bool {
	return allocation.sourcePalletsSet || allocation.SourcePallets > 0
}

func outboundPickAllocationHasTargetPallets(allocation OutboundPickAllocation) bool {
	return allocation.targetPalletsSet || allocation.TargetPallets > 0
}

func deriveOutboundSourcePallets(totalPallets int, availableQty int, allocatedQty int) int {
	if totalPallets <= 0 || allocatedQty <= 0 {
		return 0
	}
	if availableQty <= 0 || allocatedQty >= availableQty {
		return totalPallets
	}
	pallets := int(math.Ceil(float64(totalPallets) * float64(allocatedQty) / float64(availableQty)))
	if pallets < 1 {
		return 1
	}
	if pallets > totalPallets {
		return totalPallets
	}
	return pallets
}
