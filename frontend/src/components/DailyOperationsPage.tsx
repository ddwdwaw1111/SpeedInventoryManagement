  ...
  return (
    <section className="rounded-[24px] border border-slate-200/80 bg-white p-4 shadow-[0_16px_34px_rgba(15,23,42,0.05)]">
      ...
      {isLoading ? (
        ...
        <div className="mt-4 space-y-3">
          {rows.map((row) => {
            const isAdvanceBusy = busyActionKey === `advance-${row.rowKey}`;
            const isCopyBusy = busyActionKey === `copy-${row.rowKey}`;
            const isRowBusy = isAdvanceBusy || isCopyBusy;
